...
struct CueGeneratorView: View {
    ...
    private var fileSelectionSection: some View {
        SectionContainer(title: "Source Files", systemImage: "doc.on.doc") {
            VStack(spacing: 16) {
                if scanState.binFiles.isEmpty {
                    VStack(spacing: 12) {
                        ...
                        Text("Select .bin files")
                            .font(.headline)
                            .foregroundStyle(.secondary)

                        Text("A single file (multi-track) or multiple files (one per track)")
                            .font(.caption)
                        ...
                        HStack(spacing: 12) {
                            Button {
                                ...
                                Label(".bin Files", systemImage: "doc.on.doc")
                            }
                            .buttonStyle(.glass)
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("\(scanState.binFiles.count) .bin file(s) selected")
                                .font(.headline)

                            Text(String(format: "Total size: %.1f MB", scanState.totalSizeMB))
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                        }
    ...
    private var tracksSection: some View {
        ...
        return SectionContainer(title: "Tracks detected (\(dataCount) data, \(audioCount) audio)", systemImage: "music.note.list") {
            VStack(spacing: 0) {
    ...
    private var cuePreviewSection: some View {
        SectionContainer(title: "CUE Preview", systemImage: "doc.text") {
                ...
                Text(scanState.generatedCue)
                    ...
                    .contextMenu {
                        Button {
                            ...
                            Label("Copy CUE content", systemImage: "doc.on.doc")
                        }
                    }
            }
        }
        ...
        GlassEffectContainer(spacing: 12) {
            HStack(spacing: 12) {
                Button {
                    ...
                    Label("Save .cue", systemImage: "square.and.arrow.down")
                        .frame(maxWidth: .infinity)
                }
                ...
                Button {
                    ...
                    Label("Save and burn", systemImage: "flame")
                        .frame(maxWidth: .infinity)
                }
                .buttonStyle(.glass)
                .controlSize(.large)
    ...
    private func selectFiles() {
        ...
        panel.message = "Select one or more .bin files"
    ...
    private func selectFolder() {
        ...
        panel.message = "Select a folder containing .bin files"
        ...
        if panel.runModal() == .OK, let url = panel.url {
            ...
            if scanState.binFiles.isEmpty {
                scanState.errorMessage = "No .bin files found in this folder."
            }
        }
    }
    ...
    private func saveCueFile() {
        ...
        if panel.runModal() == .OK, let url = panel.url {
            do {
                ...
                scanState.errorMessage = "Error saving file: \(error.localizedDescription)"
            }
        }
    }
